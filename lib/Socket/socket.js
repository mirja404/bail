...
const handleIncomingMessage = (messageNode: any) => {
    try {
        const messages = extractMessages(messageNode);
        messages.forEach(msg => {
            logger.info(`Message received from ${msg.from}: ${msg.body}`);

            // Contoh: Auto-reply pesan teks
            if (msg.type === 'text') {
                sendMessage(msg.from, `You said: ${msg.body}`);
            }
        });
    } catch (err) {
...
const sendMessage = async (to: string, text: string) => {
    ...
    await sendNode('message', messageNode);
    logger.info(`Message sent to ${to}: ${text}`);
};
